...
export function HeroMarquee() {
  ...
  return (
    <div className="overflow-hidden bg-gradient-primary py-2.5 md:hidden">
      <div className="flex w-max animate-marquee">
        {[0, 1].map((copy) => (
          <div key={copy} className="flex flex-none items-center" aria-hidden={copy === 1}>
            {items.map((item, i) => (
              <span key={i} className="flex items-center whitespace-nowrap px-4 text-xs font-bold uppercase tracking-[0.08em] text-white">
                {item}
