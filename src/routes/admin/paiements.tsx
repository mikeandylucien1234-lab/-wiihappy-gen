...
function NewPaiementModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="mb-5 text-lg font-extrabold text-ink">{p.modalTitle}</h3>

        <div className="flex flex-col gap-4">
          <div>
            <Label>{p.devisLabel}</Label>
            <Select value={devisId} onChange={(e) => setDevisId(e.target.value)}>
              <option value="">{p.devisPlaceholder}</option>
              {candidates?.map((d) => (
            ...
            {candidates?.length === 0 && <p className="mt-1.5 text-xs text-slate">{p.noCandidates}</p>}
          </div>
          ...
          <div className="grid grid-cols-[2fr_1fr] gap-3">
            ...
            <div>
              <Label>{p.currencyLabel}</Label>
              <Input value={currency} onChange={(e) => setCurrency(e.target.value)} placeholder="EUR" />
            </div>
          </div>

          <div>
            <Label>{p.referenceLabel}</Label>
            <Input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="WH-2026-0042" />
          </div>
          <div>
            <Label>{p.ibanLabel}</Label>
            <Input value={iban} onChange={(e) => setIban(e.target.value)} placeholder="FR76 3000 6000..." />
          </div>
          <div>
            <Label>{p.bicLabel}</Label>
            <Input value={bic} onChange={(e) => setBic(e.target.value)} placeholder="AGRIFRPP" />
          </div>
          <div>
            <Label>{p.dueDateLabel}</Label>
            <Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
          </div>
          <div>
            <Label>{p.instructionsLabel}</Label>
            <Input value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder={p.instructionsPlaceholder} />
          </div>

          {createPaiement.isError && <p className="text-sm font-semibold text-danger">{p.createError}</p>}
        </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <Button type="button" variant="accent" onClick={handleSubmit} disabled={!devisId || !amount || createPaiement.isPending}>
            {createPaiement.isPending ? p.creating : p.create}
          </Button>
        </div>
      </div>
    </div>
...
function AdminPaiements() {
  ...
  return (
    <div>
      ...
      {isError && (
        <p className="rounded-md bg-danger/[0.08] px-3 py-2.5 text-sm font-semibold text-danger">{p.error}</p>
      )}
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[860px]">
              <div className="grid grid-cols-[1.3fr_0.9fr_0.9fr_1fr_0.8fr_0.6fr] gap-3 border-b border-navy/[0.08] px-5 py-3.5 text-xs font-extrabold uppercase tracking-[0.03em] text-slate">
                <span>{p.colClient}</span>
                <span>{p.colAmount}</span>
                <span>{p.colReference}</span>
                <span>{p.colDueDate}</span>
                <span>{p.colDevisStatus}</span>
                <span>{p.colAction}</span>
              </div>
              {paiements.map((pm) => (
                <div
                  key={pm.id}
                ...
                >
                  <span className="truncate font-bold text-ink">{pm.devis?.name ?? '—'}</span>
                  <span className="font-semibold text-ink">
                    {pm.amount.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {pm.currency}
                  </span>
                  <span className="truncate text-slate">{pm.reference ?? '—'}</span>
                  <span className="text-slate">
                    {pm.due_date ? new Date(pm.due_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                  </span>
                  <span>
                    {pm.devis && <Badge variant={statusBadgeVariant[pm.devis.status]}>{t.devisStatus[pm.devis.status]}</Badge>}
                  </span>
                  {pm.devis && (
                    <Link to="/admin/devis/$devisId" params={{ devisId: pm.devis.id }} className="font-bold text-primary">
                      {p.view} →
                    </Link>
                  )}
                </div>
